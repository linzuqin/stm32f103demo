...
#define YBUFF_INDEX 1
#define YMODEM_USING_CRC_TABLE
//static rt_uint8_t rym_buff[1024+1+2+2+3];//+3是为了4字节对齐
...
//    rt_tick_t timeout)
//{
//    if(rt_mq_recv(ctx->dev->mq, ctx->buf, 200,timeout)==RT_EOK)
//    {
//        return (enum rym_code)(*(ctx->buf+3));
//    }
//    else
//    {
...
//    rt_device_write(ctx->dev, 0, &code, sizeof(code));
    USART_SendData(ctx->dev->USARTx, code);
    while(USART_GetFlagStatus(ctx->dev->USARTx, USART_FLAG_TC) == RESET);
    return 1;
}
...
//    rt_tick_t tick;
    ...
    for (i = 0; i < tm_sec; i++)
    {
        _rym_putchar(ctx, RYM_CODE_C); //send C
//        code = _rym_read_code(ctx,
//                              RYM_CHD_INTV_TICK);
        if(rt_mq_recv(ctx->dev->mq, ctx->buf, 200, RYM_CHD_INTV_TICK)==RT_EOK)//RYM_CHD_INTV_TICK RT_WAITING_FOREVER
        {
            code = (enum rym_code)(*(ctx->buf+YBUFF_INDEX));
        }
        else
        {
            code = RYM_CODE_NONE;
...
//    }
//    if (i != (data_sz - 1))
    if (UART1_Count<data_sz)
        return -RYM_ERR_DSZ;

    /* sanity check */
    if (ctx->buf[1+YBUFF_INDEX] != 0 || ctx->buf[2+YBUFF_INDEX] != 0xFF)
        return -RYM_ERR_SEQ;

    recv_crc = (rt_uint16_t)(*(ctx->buf+YBUFF_INDEX + data_sz - 2) << 8) | *(ctx->buf+YBUFF_INDEX + data_sz - 1);
    cal_crc = CRC16(ctx->buf+YBUFF_INDEX + 3, data_sz - 5);
    if (recv_crc != cal_crc)
        return -RYM_ERR_CRC;

    /* congratulations, check passed. */
    if (ctx->on_begin && ctx->on_begin(ctx, ctx->buf+YBUFF_INDEX + 3, data_sz - 5) != RYM_CODE_ACK)
        return -RYM_ERR_CAN;
    ...
    enum rym_code *code)
{
    ...
    /* seq + data + crc */
//    rt_size_t i = _rym_read_data(ctx, tsz); UART1_Count
    if (UART1_Count != (tsz+1))
    {
//        rt_kprintf("dotans,ERR_DSZ\r\n");
        return -RYM_ERR_DSZ;
    }

    if ((ctx->buf[1+YBUFF_INDEX] + ctx->buf[2+YBUFF_INDEX]) != 0xFF)
    {
//        rt_kprintf("dotans,ERR_SEQ\r\n");
        return -RYM_ERR_SEQ;
    }
    ...
    if (ctx->stage == RYM_STAGE_ESTABLISHED && ctx->buf[1+YBUFF_INDEX] == 0x00)
    {
        *code = RYM_CODE_NONE;
        return RT_EOK;
    }
    ...
    /* sanity check */
    recv_crc = (rt_uint16_t)(*(ctx->buf+YBUFF_INDEX + tsz - 1) << 8) | *(ctx->buf+YBUFF_INDEX + tsz);
    if (recv_crc != CRC16(ctx->buf+YBUFF_INDEX + 3, data_sz))
    {
//        rt_kprintf("dotans,ERR_CRC\r\n");
        return -RYM_ERR_CRC;
    }

    /* congratulations, check passed. */
    if (ctx->on_data)
        *code = ctx->on_data(ctx, ctx->buf+YBUFF_INDEX + 3, data_sz);
    else
        *code = RYM_CODE_ACK;
    return RT_EOK;
}
...
//        code = _rym_read_code(ctx,
//                              RYM_WAIT_PKG_TICK);
        if(rt_mq_recv(ctx->dev->mq, ctx->buf, 200, RYM_WAIT_PKG_TICK)==RT_EOK) //RYM_WAIT_PKG_TICK
        {
            code = (enum rym_code)(*(ctx->buf+YBUFF_INDEX));
        }
        else
        ...
        case RYM_CODE_STX:
            ...
            break;
        case RYM_CODE_EOT:
//            rt_kprintf("dotans,EOT\r\n");
            return RT_EOK;
        default:
//            rt_kprintf("dotans,ERR_CODE,%x\r\n",code);
            return -RYM_ERR_CODE;
        };
...
//    rt_size_t i;
     ...
     * one. */
    if (ctx->on_end)
        ctx->on_end(ctx, ctx->buf+YBUFF_INDEX + 3, 128);
...
//    if (*(ctx->buf+2) != (_RYM_SOH_PKG_SZ - 0))
//        return -RYM_ERR_DSZ;
    ...
    recv_crc = (rt_uint16_t)(*(ctx->buf+YBUFF_INDEX + _RYM_SOH_PKG_SZ - 2) << 8) | *(ctx->buf+YBUFF_INDEX + _RYM_SOH_PKG_SZ - 1);
    if (recv_crc != CRC16(ctx->buf+YBUFF_INDEX + 3, _RYM_SOH_PKG_SZ - 5))
        return -RYM_ERR_CRC;

    /*next file transmission*/
    if (ctx->buf[3+YBUFF_INDEX] != 0)
    {
        if (ctx->on_begin && ctx->on_begin(ctx, ctx->buf+YBUFF_INDEX + 3, data_sz - 5) != RYM_CODE_ACK)
            return -RYM_ERR_CAN;
        return RT_EOK;
...
//    ctx->buf = rt_malloc(_RYM_STX_PKG_SZ);
    ...
    {
//        rt_free(ctx->buf);
//        rt_kprintf("handshake fail");
        return err;
    }
//    rt_kprintf("handshake");
    while (1)
    {
...
//    _rym_the_ctx = ctx;
    ...
    res = _rym_do_recv(ctx, handshake_timeout);//handshake_timeout
